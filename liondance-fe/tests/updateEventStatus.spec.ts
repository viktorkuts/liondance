...
test('can edit specific event status to CONFIRMED', async ({ page }) => {
  ...
  await page.getByRole('row', { name: 'Rio Da Yung OG rioodayungog@' }).getByRole('button').click();
  ...
  await page.getByRole('button', { name: 'Update Status' }).click();
  await expect(page.getByLabel('Update Event Status')).toContainText('Failed to update event status, please provide a valid choice.');
  await page.getByRole('textbox', { name: 'Event Status' }).click();
...
test('can edit specific event status to PENDING', async ({ page }) => {
    ...
    await page.getByRole('row', { name: 'Chief Keef lovesosa@' }).getByRole('button').click();
    await page.getByRole('textbox', { name: 'Event Status' }).click();
    await page.getByRole('option', { name: 'PENDING' }).click();
    await page.getByRole('button', { name: 'Update Status' }).click();
    await expect(page.locator('tbody')).toContainText('PENDING');
...
test('can edit specific event status to CANCELLED', async ({ page }) => {
  await page.goto('http://localhost:5173/events');
  await expect(page.locator('tbody')).toContainText('PENDING');
  await page.getByRole('row', { name: 'Joshua Block joshblock@' }).getByRole('button').click();
  await page.getByRole('textbox', { name: 'Event Status' }).click();
