...
test('can reschedule an event at thier will', async ({ page }) => {
  ...
  await page.locator('div').filter({ hasText: /^2025$/ }).getByRole('button').nth(2).click();
  ...
  await page.locator('div').filter({ hasText: /^New Time$/ }).getByRole('textbox').click();  await page.getByRole('option', { name: '7:00 PM' }).click();
  await page.getByLabel('Reschedule Event').getByRole('button', { name: 'Reschedule' }).click();
