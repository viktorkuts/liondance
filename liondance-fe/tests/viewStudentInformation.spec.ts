...
test('can view student information', async ({ page }) => {
    ...
    await expect(page.locator('tbody')).toContainText('Nikolaos Georgios Michaloliakos');
    await page.getByRole('row', { name: 'Nikolaos Georgios' }).getByRole('button').click();
    await expect(page.locator('#root')).toContainText('Name: Nikolaos Georgios Michaloliakos');
