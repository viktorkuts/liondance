...
const UpdateEventStatus: React.FC<UpdateEventStatusProps> = ({ event, onClose, onUpdate }) => {
 ...
 const handleUpdateStatus = async () => {
  setLoading(true);
  try {
   const response = await axiosInstance.patch<Event>(`/events/${event.id}/status`, { eventStatus: newStatus });
 ...
 return (
  <Modal opened={true} onClose={onClose} title="Update Event Status">
   ...
   <Select
    ...
    data={Object.values(EventStatus).map((status) => ({ value: status, label: status }))}
   />
